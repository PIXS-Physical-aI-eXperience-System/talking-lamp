...
| 서보 ID | 관절 이름 | 범위 (MuJoCo, 실기기 캘리브레이션 기준) |
| --- | --- | --- |
| 1 | `base_yaw` | -91.9° … 85.1° |
| 2 | `base_pitch` | -121.5° … 66.0° |
| 3 | `elbow_pitch` | -165.4° … 31.0° |
| 4 | `wrist_roll` | -90.4° … 91.4° |
| 5 | `wrist_pitch` | -54.3° … 143.5° |

표는 `HardwareAlignment.load().joint_limits`를 도 단위로 환산해 소수점 한 자리로
반올림한 값이다. 원본은 `hardware_alignment.json`의 실측 엔코더 범위·수직 기준·방향이며,
`build_arm.py`도 같은 값을 MJCF에 기록한다.
...
home 자세에서 헤드 팁 ≈ `(0.184, -0.005, 0.347) m`.
현재 캘리브레이션에서의 단일 관절 스윕 도달 범위는 `make check`로 확인한다.
...
1. 깨진 모델을 로드해 **CAD 영점 자세(전 관절 0)에서** 각 관절의 월드 앵커·축과
   각 구조 메시의 월드 트랜스폼을 읽는다. 가동범위는 `hardware_alignment.json`의
   실측값으로 교체한다. CAD 영점 자세는 현재의 굽힌 `home` 대기 자세와 다르다.
2. 이 값들로 **직렬 체인**을 새로 생성: `base → base_yaw → base_pitch → elbow_pitch →
   wrist_roll → wrist_pitch → head`. 각 관절은 CAD 영점 앵커에 실제 축으로 배치,
   각 링크 프레임은 CAD 영점에서 월드 정렬 → 이 자세의 메시 배치와 렌더를 보존한다.
...
- **관절 범위는 현재 조립된 램프의 실측값** — 기구·서보를 변경하면 다시 캘리브레이션해야 한다.

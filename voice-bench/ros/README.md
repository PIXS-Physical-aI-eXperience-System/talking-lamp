...
| `앞 재생이 3초 안에 안 끝난다 — 취소하고 다음으로 넘어간다` | 앞 재생 취소가 느리다. 다음 턴을 먼저 시작한다 |
| `… — 지난 재생` | 앞 재생의 결과가 늦게 왔다. 지금 재생은 건드리지 않는다 |
| `PlayAudio 거부됨` | 파이가 목표를 거부(보통 `audio_busy`). 끝난 것으로 처리한다 |

판단부가 `! 재생 완료 신호가 안 왔다 (N초 기다림)` 을 찍으면 노드가 결과를
못 준 것이다. 노드가 죽었거나 구버전이다 — 보낸 오디오 길이 + 8초까지만
기다리고 대기로 간다.
...
- **`playback_frames` 구독을 `MutuallyExclusiveCallbackGroup` 으로.** 지금은
  ...
  그쪽에서 막으면 근본적으로 해결된다.
- 젯슨 `~/talking-lamp-integration` 에 커밋 안 된 수정이 돌고 있다.

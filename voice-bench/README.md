...
| `melo_export_bert.py` | 한국어 BERT → ONNX |
| `melo_quantize.py` | 위 둘을 int8로 양자화 |
| `melo_text/` | torch를 쓰지 않는 프론트엔드 사본 ([변경 내역](THIRD-PARTY-NOTICES.md)) |
| `runners/tts_melo_onnx.py` | ONNX 전용 추론 |
| `ko_normalize.py` | 숫자·영문을 한글 읽기로 (`15도`→`십오 도`, `3시`→`세 시`) |
...
```bash
python melo_export_onnx.py      # VITS  → melo-ko-vits.onnx
python melo_export_bert.py      # BERT  → bert-kor-base.onnx + tokenizer/ + frontend.json
python melo_quantize.py         # 둘 다 → *.int8.onnx
```
...
- `models/melo-ko-onnx/` 폴더만 복사하면 된다. 내보내기를 다시 할 필요 없다.

...
| `--ready-wait` | 0.6 | 수락 후 첫 프레임까지 기다리는 시간. 브리지가 준비 신호를 안 줘서 짐작이다 |
| `--pi-host` | | 쓰지 않는다. 넘겨도 무시한다(띄우는 명령이 깨지지 않게 남겨 둠) |
...
| `e2e-2026-09-21.md` | 전 구간 실측 — 지연·barge-in·버그 넷, PR 리뷰 반영과 전체 점검(경합 일곱) |
...
- **재생·취소를 실기기에서 확인** — 하드웨어 없이는 전부 시험했다. 끼어들면
  스피커가 실제로 멈추는지, 취소가 느릴 때 다음 재생이 `audio_busy` 없이 나오는지,
  반복 중 `audio_busy`·`out_of_order` 가 안 나는지. 목록은
  [ros/README.md](ros/README.md) "아직 안 된 것"

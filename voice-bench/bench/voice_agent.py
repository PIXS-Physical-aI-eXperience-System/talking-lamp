...
답을 만드는 것은 --llm 으로 붙인다(OpenAI 호환, 스트리밍). 모델은 아직
정하지 않았고, 주지 않으면 들은 말을 되받아 말한다.
"""
import argparse
import os
import socket
...
from voice.agent import BARGE_RISE_DB, VoiceAgent   # noqa: E402
...
def _pump(sock, agent):
    while True:
        ...
        if kind == link.CAP_FRAME:
            ...
            agent.on_play_done(sid, code.decode("utf-8", "replace"))
        elif kind == link.PING:

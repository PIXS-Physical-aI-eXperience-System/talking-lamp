...
def cmd_calibrate(args):
    ...
    print("[2/2] 한쪽 옆")
    print("  마이크를 마주 본 채로 한쪽으로 확실히 옮기세요 (45° 이상이면 충분).")
    print("  정확히 90° 일 필요는 없습니다 — 여기서는 '어느 쪽이 +인지'만 정합니다.")
    # '램프 기준 왼쪽' 처럼 물으면 사용자가 시점을 뒤집어 생각해야 하고, 거기서
    # 틀리면 부호가 반대로 저장된다. 본인 손을 기준으로 물으면 뒤집을 일이 없다.
    side = ""
    while side not in ("l", "r"):
        side = input("  마이크를 마주 본 상태에서 어느 손 쪽으로 가셨습니까? "
                     "오른손=r / 왼손=l : ").strip().lower()
    ...
    # 램프의 왼쪽이 +90° 다. 마이크를 마주 본 사람의 오른손 쪽이 곧 램프의
    # 왼쪽이므로, '오른손 쪽으로 갔을 때 원시값이 커진다' 면 부호가 +1 이다.
    # 여기를 뒤집으면 오차표는 멀쩡해 보이는데 램프가 소리 반대쪽으로 돈다.
    ...
    print("     확인: 마이크를 마주 보고 오른손 쪽으로 가면 +, 왼손 쪽으로 가면 −")

    # 참고용. 이동각을 정확히 모르므로 판정하지 않고 알려만 준다.
    # 원시 각도가 실제 각도에 비례하는지는 measure 결과로 확인한다.
    print(f"  (참고: {'오른손' if side == 'r' else '왼손'} 쪽으로 옮겼을 때 원시값이 "

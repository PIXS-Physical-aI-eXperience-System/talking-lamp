...
stt() {
  ...
  for dev in cuda cpu; do
    ...
    TOKENIZERS_PARALLELISM=false "$V/bin/python" bench/runners/stt_faster_whisper.py \
      ...
      --ref-dir "$ROOT/ref" --label "fw-small-$dev" 2>&1 \
      | grep -o '@@RESULT@@.*' | "$V/bin/python" -c "
import json,sys
s=sys.stdin.read().replace('@@RESULT@@','')
if not s.strip(): print('    실패 — 위 오류 확인'); raise SystemExit
d=json.loads(s)
...
tts() {
  for mode in int8 fp32; do
    ...
    TOKENIZERS_PARALLELISM=false "$V/bin/python" runners/tts_melo_onnx.py \
      --out-dir "$ROOT/out/tts/jetson-$mode" --label "melo-onnx-$mode" --normalize $flag 2>&1 \
      | grep -o '@@RESULT@@.*' | "$V/bin/python" -c "
import json,sys
s=sys.stdin.read().replace('@@RESULT@@','')
if not s.strip(): print('    실패 — 위 오류 확인'); raise SystemExit

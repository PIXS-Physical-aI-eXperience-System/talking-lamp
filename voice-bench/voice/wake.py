...
THRESHOLD = 0.7     # 실측에서 고른 자리 — results/wake-2026-09-17.md
NEED_FRAMES = 3     # 연속 3창
...
class OpenWakeWord:
    ...
    **연속 세 창을 요구한다.** 한 창만 넘어도 깨우면 스치는 소리에 깨어난다.
    실제 마이크로 잰 5분 대화(bench/wake_field.py):

        연속 1창, 임계 0.7 — 헛깨움 2.20회/분
        연속 2창, 임계 0.7 — 헛깨움 1.20회/분
        연속 3창, 임계 0.7 — 헛깨움 0.40회/분

    임계를 0.9 로 올려도 헛깨움은 0.40회/분 으로 같은데 깨어남만 떨어져서
    (팀원 99→98%, 학습 밖 목소리 42→26%) 임계는 0.7 에 둔다.

    팀원 목소리는 임계 0.7 을 중앙값 9창 연속으로 넘으므로 3창을 요구해도
    거의 안 떨어진다(100→99%). 학습 밖 목소리는 여유가 없어 크게 떨어진다 —
    그쪽은 임계값으로 풀 문제가 아니다.

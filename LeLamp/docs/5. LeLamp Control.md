...
sudo uv run -m lelamp.test.check_motors --id your_lamp_name --port your_port

# Test head components
sudo uv run -m lelamp.test.check_rgb     # LED ring
uv run -m lelamp.test.check_audio        # Audio I/O
